...
double get_torque(double theta, double dtheta, double t, double num_e, double w)
{
    // Make sure theta0 is initialized once
    if (theta0 == 0)
    {
        theta0 = theta;
    }

    // num_e influences the torque, changing for each simulation run
...
void simulate_and_save(double num_e, const std::string &weight_label)
{
    ...
    while (t <= TIME_LIMIT)
    {
        ...
        double torque = get_torque(theta, dtheta, t, num_e, w);  // num_e dynamically affects the torque here
...
void run_simulation(const std::string &weight_label)
{
    ...
    for (double num_e = 3.0; num_e <= 15.0; )
    {
        ...
        simulate_and_save(num_e, weight_label);  // Pass num_e correctly to affect the simulation
